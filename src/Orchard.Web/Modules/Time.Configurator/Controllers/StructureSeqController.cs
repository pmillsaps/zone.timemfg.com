...
namespace Time.Configurator.Controllers
{
    ...
    public class StructureSeqController : Controller
    {
        ...
        private void GenerateDropDowns()
        {
            ...
            ViewBag.Lookup = new SelectList(LookupList.ToList(), "Lookup", "Lookup");
            ViewBag.LookupSequence = new SelectList(LookupSequenceList.ToList(), "LookupSequence", "LookupSequence");
        }

        private void GenerateDropDowns(StructureSeq structureseq)
        {
            ViewBag.ConfigName = new SelectList(db.StructureSeqs.OrderBy(x => x.ConfigName), "ConfigName", "ConfigName", structureseq.ConfigName);
            ViewBag.ConfigData = new SelectList(db.StructureSeqs.OrderBy(x => x.ConfigData), "ConfigData", "ConfigData", structureseq.ConfigData);

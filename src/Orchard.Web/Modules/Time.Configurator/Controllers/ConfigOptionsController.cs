...
namespace Time.Configurator.Controllers
{
    ...
    public class ConfigOptionsController : Controller
    {
        ...
        public ActionResult Index(string display, string ConfigNames, string ConfigData)
        {
            ...
            ViewBag.ConfigData = new SelectList(db.ConfigOptions.Select(x => x.ConfigData).Distinct());
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "Id,ConfigName,ConfigData,Key01,Key02,Key03,Key04,Key05,Key06,Key07,Key08,Key09,Key10,PartNum,InActive")] ConfigOption configOption)
        {
            if (ModelState.IsValid)
            {

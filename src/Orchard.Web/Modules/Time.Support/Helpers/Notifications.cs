...
namespace Time.Support.Helpers
{
    ...
    public abstract class EmailNotifications : INotifications
    {
        ...
        public void AddEmailSendToBcc(string e)
        {
            ...
            if (sr == null) sr = new SettingRepository();
        ...
        public void AddSupportAdminSendTo(string e)
        {
            //var sendTo = new List<string>(ConfigurationManager.AppSettings["emailSendTo"].Split(','));
            if (sr == null) sr = new SettingRepository();
            var sendTo = new List<string>(sr.GetSettings("SupportAdministrator").Split(','));
            // Section for CC'ing ITAdmin group
            foreach (var email in sendTo)
            {
                if (e != email)
        ...
        private void BuildEmailBody()
        {
            ...
            using (var path = new StreamReader(TemplatePath))
            {
                ...
                var blah = path.ReadToEnd();
                string ticketStatus = "";
                if (tp.TicketStatus != null) ticketStatus = tp.TicketStatus.Name;
    ...
    public class ApprovedNotification : EmailNotifications
    {
        ...
        public override void SetupEmail()
        {
            ...
            var e = GetEmailforNTUser(tp.RequestedBy);
            AddSupportAdminSendTo(e);
            AddEmailSendToBcc(e);
            if (tp.TicketStatus.isSupRequest)
                Subject = String.Format("[Ticket #{0}] has been approved by the supervisor - {1}", tp.TicketID, tp.Title);
            else
                Subject = String.Format("[Ticket #{0}] has NOT been approved by the supervisor and is now closed - {1}", tp.TicketID, tp.Title);
        }
    }
